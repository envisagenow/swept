...
namespace swept
{
    public class BuildReporter
    {
        ...
        public string ReportDetailsXml(RuleTasks ruleTasks, int limit, int runNumber, List<Pick> picks)
        {
            ...
            if (picks != null) 
            {
                var tags = picks.Where(p => p.Domain == PickDomain.Tag).Select(p => p.Value);
                if (tags.Any())
